...
public static class ContentExtensions
{
    public class PublishedContentWrapper : IPublishedContent
    {
            ...
            IContentService contentService, IPublishedModelFactory publishedModelFactory, PublishedContentConverter converter)
        {
            ...
            {
                var ct = ctBaseService.Get(_inner.ContentType.Id) ?? throw new ArgumentNullException();
            ...
            _properties = new Lazy<IPublishedProperty[]>(() => ContentType.PropertyTypes
                .Select(x =>
                {
                    var p = _inner.Properties.SingleOrDefault(xx => xx.Alias == x.Alias) ?? throw new ArgumentNullException();
            ...
            _children = new Lazy<IEnumerable<IPublishedContent>>(() =>
            {
                ...
                foreach (var record in contentService.GetPagedChildren(_inner.Id, 0, 2000000000, out var totalRecords))
                {
                    var node = converter?.ToPublishedContent(record, culture)?.CreateModel(publishedModelFactory);
                    if (node != null)
                    {
                        list.Add(node);
                    }
        ...
        public string Name
            => _inner.Name ?? _inner.Id.ToString();
        ...
        public IPublishedProperty GetProperty(string alias)
            => _properties.Value.FirstOrDefault(x => x.Alias.InvariantEquals(alias)) ?? throw new ArgumentOutOfRangeException(alias);
