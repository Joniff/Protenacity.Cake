...
import { PosmMap } from "./posm-map";
import './posm-openstreetmap-autosuggest-element';
import { UUIInputElement } from '@umbraco-cms/backoffice/external/uui';
import { PosmOpenStreetMapElement } from "./posm-openstreetmap-element";     
export * from './posm-openstreetmap-element'

@customElement('posm-openstreetmap-property-editor')
export default class PosmOpenStreetMapElementPropertyEditorUI extends UmbElementMixin(LitElement) {

    @property()
    value: Partial<PosmMap> = {};
    ...
    @state()
    private _defaultPosition?: PosmMap; /** BE AWARE THAT THIS IS ACTUALLY DEFAULT VALUE */
    ...
    async firstUpdated(changes:any) {
        ...
        if(this.value?.marker) { 
            this.inputLat = this.value.marker?.latitude.toFixed(4);
            this.inputLng = this.value.marker?.longitude.toFixed(4);
        }
    }
    ...
    updateData(e: CustomEvent) {
        const mapEle = e.target as PosmOpenStreetMapElement;
        this.value = {
                ...
                (this._showCoordinates || this._allowClear ) && this.value?.marker
                    ? html`
                        <div style="clear:both;">
                            <umb-localize key="osm_latitude"></umb-localize>: ${this.value.marker?.latitude.toFixed(4) }, 
                            <umb-localize key="osm_longitude"></umb-localize>: ${this.value.marker?.longitude.toFixed(4) }
