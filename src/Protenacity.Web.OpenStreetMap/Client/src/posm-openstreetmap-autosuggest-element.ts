...
@customElement('posm-openstreetmap-auto-suggest')
export class PosmOpenStreetMapAutoSuggestElement extends LitElement {
    ...
    async onEntry(event: any) {
        ...
        this.list.innerHTML = this.data.map(obj => `<option value="${obj.name}" data-obj='${obj ? JSON.stringify(obj) : ''}'>${obj.name}</option>`).join('')
