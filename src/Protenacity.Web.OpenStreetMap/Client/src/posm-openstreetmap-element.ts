...
import { PosmBoundingCoordinates } from './posm-bounding-coordinates';
import { PosmCoordinate } from './posm-coordinate';

const DEFAULT_BOUNDING_BOX = { "southWestCorner": { "latitude": 54.970495, "longitude": -1.627864 }, "northEastCorner": { "latitude": 54.97911, "longitude": -1.609625 } };
...
@customElement('posm-openstreetmap')
export class PosmOpenStreetMapElement extends LitElement {
  ...
  `];
    ...
    private _boundingBox: PosmBoundingCoordinates = DEFAULT_BOUNDING_BOX;
    set boundingBox(boundingBox: PosmBoundingCoordinates) {
    ...
    private _markerLocation!: PosmCoordinate;
    set markerLocation(location: PosmCoordinate) {
    ...
    private fitBounds(boundingBox: PosmBoundingCoordinates) {
        this._boundingBox = boundingBox;
    ...
    private updateMarker(position: PosmCoordinate, first?: boolean) {
        if (this._marker) {
            this.clearMarker();
        }
        if (position) {
    ...
    private onMapClick(e: LeafletMouseEvent) {
        this.updateMarker({ latitude: e.latlng.lat, longitude: e.latlng.lng } as PosmCoordinate);
    }
    private onMoveEnd() {
        this._boundingBox = {
            northEastCorner: {
    ...
    render() {
        return html`<link rel="stylesheet" href="https://unpkg.com/leaflet@${L.version}/dist/leaflet.css" /><div class="map"></div>`;
    }
